...
public class GuiUtil extends Gui {
    ...
    public static void drawChromaStringAstolfo(String textIn, int xIn, int y, boolean shadow) {
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        for (int j = 0; j < (textIn.split("\n")).length; j++) {
            int x = xIn;
            for (char c : textIn.split("\n")[j].toCharArray()) {
                long l = System.currentTimeMillis() - x * 10L - (y - j * 10L) * 10L;
                float speed = 2000.0F;
                int color = Color.HSBtoRGB((float)(l % (int)speed) / speed, 0.6F, 0.8F);
                drawString(String.valueOf(c), x, y + j * (mc.fontRendererObj.FONT_HEIGHT + 2), color, shadow);
                x += mc.fontRendererObj.getStringWidth(String.valueOf(c));
            }
    ...
    public static void drawChromaStringModern(String textIn, int xIn, int y, boolean shadow) {
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        for (int j = 0; j < (textIn.split("\n")).length; j++) {
            int x = xIn;
            for (char c : textIn.split("\n")[j].toCharArray()) {
                ...
                float speed = 2000.0F;
                int color = Color.HSBtoRGB((float)(l % (int)speed) / speed, 0.8F, 0.8F);
                Asyncware.renderer.drawString(String.valueOf(c), x + 1, y + j * (mc.fontRendererObj.FONT_HEIGHT + 2) + 1, 0x000, false);
                Asyncware.renderer.drawString(String.valueOf(c), x, y + j * (mc.fontRendererObj.FONT_HEIGHT + 2), color, false);


                // Base.fontxd.drawString(String.valueOf(c), x, y + j * (mc.fontRendererObj.FONT_HEIGHT + 2), color, shadow);
                x += Asyncware.renderer.getStringWidth(String.valueOf(c));
            }
        }
    }
